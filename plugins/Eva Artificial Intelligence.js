...
Meda.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        var payload_client = await WhatsMedaStack.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'Ber4tbey', 'WhatsMeda', unique_ident)
        var fins = ''                           
        if (conf.LANG !== 'EN') {
...
Meda.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (eva_functionality == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
            ...
            if (message.jid.includes('-') && (message.mention !== false && message.mention.length !== 0)) {
                ...
                var payload_client = await WhatsMedaStack.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'HardDecode', 'WhatsMeda', unique_ident)
                var fins = ''                           
                if (conf.LANG !== 'EN') {
                    ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                    if ('text' in ceviri) {
...
Meda.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc, fromMe: wk }, (async (message, match) => {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                let acc = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0] == 'Meda' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'       
                var ainame = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0]
                if (ainame !== 'Meda') return;
                var payload_client = await WhatsMedaStack.get_eva_ai(encodeURIComponent(ssc), 'Eva', 'Ber4tbey', 'WhatsMeda', unique_ident)
                var fins = ''                           
                if (conf.LANG !== 'EN') {
                    ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                    if ('text' in ceviri) {
...
Meda.addCommand({ pattern: 'fulleva ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
